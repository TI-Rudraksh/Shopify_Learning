...
public sealed class HandleOrderCreatedCommandHandler
    : IRequestHandler<HandleOrderCreatedCommand, Unit>
{
    ...
    public async Task<Unit> Handle(
        HandleOrderCreatedCommand command, CancellationToken cancellationToken)
    {
        var payload = command.Payload;

        if (await _webhookEvents.ExistsProcessedAsync(
                "orders/create", payload.Id, cancellationToken))
            return Unit.Value;
